...
async def test_get_dishes(
    ...
    response = await client.get(
        URLS['dish']['get_by_category'].format(category=category),
    )
    ...
    assert response_data[1]['category'] == category
    assert response_data[1]['restaurant_id'] == restaurant_id
    assert response_data[1]['dish_name'] == dish_name
    assert response_data[1]['description'] == description
    assert response_data[1]['price'] == price
